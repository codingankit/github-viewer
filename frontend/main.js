...
class Profile {
  ...
  static async fetch(username) {
    try {
      ...
      const apiForFetchProfile = "/api/fetchProfile";
  ...
  static show({
    ...
    if (repos.length !== 0) {
      totalReposBadge.innerText = UtilityToolkit.formatNumber(repos.length);
      repos.forEach((repo) => {
...
class Repo {
  ...
  static async fetch({ repoName, username }) {
    try {
      ...
      const apiForFetchRepo = "/api/fetchRepo";
...
class Branches {
  ...
  static async fetch({ username, repoName }) {
    try {
      ...
      const apiForFetchBranches = "/api/fetchBranches";
...
class Files {
  ...
  static async fetch({ username, repoName, path, currentBranchForRepo }) {
    try {
      ...
      const apiForFetchFiles = "/api/fetchFiles";
  ...
  static show(files, subFilesDir) {
    ...
    files.forEach((file) => {
      if (file.type === `dir`) {
        ...
        nameIconDivDirDiv.setAttribute(`data-downloadUrl`, file.downloadUrl);
    ...
    files.forEach((file) => {
      if (file.type === `file`) {
        ...
        nameIconDivFileDiv.setAttribute(`data-downloadurl`, file.downloadUrl);

        // Create a span element for the file icon
        const fileIconSpan = document.createElement(`span`);
        fileIconSpan.classList.add(`material-symbols-rounded`, `fileIcon`);
...
class Codes {
  ...
  static show({ fileName, fetchedCode, nameIconDivFile }) {
    ...
    if (nameIconDivFile.parentElement.parentElement.id === "filesContainer") {
      ...
      function activeAllParentFolder(subFilesDiv) {
        ...
        if (
          ...
          activeAllParentFolder(subFilesDiv.parentElement.parentElement);
        }
      }
...
filesContainer.addEventListener("click", async (e) => {
  ...
  if (
    ...
    if (!subFilesDir.parentElement.children[0].classList.contains("active")) {
      subFilesDir.children[0].style.display = "block";
